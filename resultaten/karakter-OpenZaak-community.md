...
# Het karakter van de OpenZaak community en de rol van codebase stewardship

Dit document bevat de initiele reflecties op het karakter van de OpenZaak community en de rol van codebase stewardship binnen OpenZaak die tijdens het traject naar voren zijn gekomen vanuit de stakeholders.
...
Opensourcegemeenschappen verschillen sterk van karakter. Sommige bestaan uit samenwerkingen tussen grote multinationals, andere uit vrijwillige individuen, en weer andere uit gemeenschappen die samenwerken rondom een product van één bedrijf. Elk van deze modellen kent zijn eigen dynamiek en governancevorm.

OpenZaak is een duidelijk afgebakende component in een componenten-architectuur, specifiek ontwikkeld voor de Nederlandse context, onder beheer van de overheid, met een beperkt aantal marktpartijen (*het ecosysteem*). Het doel is (momenteel) niet zo veel mogelijk bijdragen van zo veel mogelijk partijen met de codebase als primair coördinatiepunt.

De nadruk ligt op ecosysteembeheer: gecoördineerd opdrachtgeverschap met één (of in de toekomst meerdere) marktpartij(en), waarbij afstemming tussen componenten essentieel is.

Dit verklaart waarom deze gemeenschap geen typische open community governance kent. De ambitie is bovendien om de aansturing in de toekomst vanuit landelijke regie te organiseren en de componenten gezamenlijk te exploiteren, en zo de samenhang tussen componenten te waarborgen.
...
In dergelijke situaties is *codebase stewardship* gericht op het beschermen van de belangen van de codebase en de gemeenschap als geheel, als tegenwicht tegen de individuele belangen van deelnemers.

Als voorbeeld: open source databasesoftware MariaDB wordt gebruikt door partijen als Amazon, Tencent, Alibaba en Microsoft. Zij profiteren van elkaars investeringen in verbeteringen, terwijl zij tegelijkertijd elk hun eigen commerciële agenda hebben. De MariaDB Foundation (met een bestuur aangewezen door de betrokken bedrijven) beheert de codebase vanuit een relatief onafhankelijke positie om machtsconcentratie te voorkomen en samenwerking duurzaam te houden. Vergelijkbare modellen bestaan ook bij onder andere de Linux Foundation.
...
* er bestaan nauwe afhankelijkheden tussen de componenten; zij staan of groeien niet volledig zelfstandig;
* sommige componenten implementeren Nederlandse wet- en regelgeving en zijn daarmee territoriaal gebonden en specifiek voor Nederlandse gemeenten;
...
* regie over het leveranciers-ecosysteem (samenwerking tussen ontwikkelpartij, reviewpartij, onderhoudspartij en SaaS-aanbieder, inclusief contract- en relatiemanagement);
* het beoordelen van het strategische afhankelijkheid en de volwassenheid van componenten om de benodigde mate van redundantie en bedrijfszekerheid te bepalen (bijvoorbeeld een tweede leverancier);
...
Wanneer landelijke regie en gezamenlijk exploiteren de uitgangspunten zijn, is de klassieke kernwaarde van codebase stewardship – het beschermen van de codebase tegen de invloed van één dominante partij – geen primair optimalisatiedoel. Er is immers één duidelijk leidend belang: dat van de landelijke regie.

Wel kan ruimte worden gelaten voor deelname door andere partijen, bijvoorbeeld andere Nederlandse of buitenlandse overheidsinstellingen, voor zover het de moeite waard is om hun belangen te bemiddelen met die van de landelijke regieorganisatie. In dat geval kan ‘vanilla’ codebase stewardship alsnog relevant zijn, maar dan misschien eerder op platformniveau (bijvoorbeeld via een entiteit als de *Stichting Platform Zaakgericht Werken*) dan op het niveau van individuele componenten.
...
* gestructureerde communicatie richting implementatie- en hostingpartners over releases, bug fixes, breaking changes en roadmaps.
